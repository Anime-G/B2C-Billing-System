...
import { Card, Carousel, Tag, Tooltip } from "antd";
...
const Home = () => {
  ...
  const getformsBills = async () => {
    const data = await axios.get(ServerApi + "/Bills/countbyuser/" + user.id)
    setBills(data.data)
  };
    ...
    <div>
      ...
      >
        {formlist.length != 0 && (
          <>
            {formlist.map((form) => {
              const c=Bills.map(bills=>{ return bills.id===form.id? bills.NoOfBills.toString():""});
              // console.log(count);
              return (
                <Link key={form.id} to={"/" + BillsNavkey + "/" + form.id}>
                  <Card hoverable="true" style={{ width: 250 }}>
                    <img draggable="false" src={form.logo} height={100} />
                    <div>
                      {" "}
                      {form.name}
                      <div style={{ position: "absolute", top:"5%", right: "1%", }}>
                      <Tooltip  placement="top" style={{justifyContent:"end"}} arrow={false} title={`Total Bills`} ><Tag color="purple" style={{fontSize:20,}} > {c}</Tag></Tooltip>
                      </div>
                    </div>
                  </Card>
                </Link>
